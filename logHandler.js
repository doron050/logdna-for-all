...
async function getLastLogs(projectToHandle) {
    ...
    if (!projectToHandle.lastSentLogTimestamp) {
        ...
        if (!projectToHandle.registrationDate) {
            console.log("no reg or date, fb to NOW!");
            projectToHandle.lastSentLogTimestamp = Date.now(); //TODO: change to -1 | 5 minutes back
        } else
            projectToHandle.lastSentLogTimestamp = projectToHandle.registrationDate;

    }
    const allLogLines = await zeit.getDeploymentsLogs(projectToHandle.projectId, projectToHandle.zeitToken, projectToHandle.teamId,projectToHandle.lastSentLogTimestamp);

   
    relevantLogs = allLogLines;
